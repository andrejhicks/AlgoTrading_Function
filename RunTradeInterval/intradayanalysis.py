...
def create_model():
    ...
    for t,ticker in enumerate(mkt.tickers):
        ...
        if (t%10==0 and t!=0) or t==mkt.tickers.shape[0]-1:
            testt.extend(traint)
            for tst_tkr in testt:
                new_data = mkt.createdata(tst_tkr)
                new_data['Symbol']=tst_tkr
                pred_data = pd.concat([pred_data,new_data])
            ...
            if passkey == 'prod' and testtickers!='':
                requests.post(f'{funcurl}?name={testtickers}&code={funckey}==',data = pred_data.to_json(orient='records'),timeout=.01)
            else:
...
def test():#main(mytimer: func.TimerRequest) -> None:
    ...
    hour = currtime.hour
    min = currtime.min
    if not requests.get('https://cloud.iexapis.com/stable/stock/twtr/quote?token={}'.format(os.environ.get("IEXProdKey"))).json()['isUSMarketOpen'] or (hour==15 and min < 25):
        dd=importmarketdata()
